...
sql_connection = 'DRIVER={SQL Server Native Client 11.0};SERVER=localhost;DATABASE=BotMain;UID=sa;PWD=wa11paper'
...
async def donate(ctx, member:discord.Member, amount = None):
    ...
    if amount == None:
        ...
        em.set_footer(text=f"Basketball Bot")
        await ctx.send(embed=em)
        return
...
# b!lb To Show The 10 Richest People In The Server ---------------------------------------------------------------------
...
# b!locker To View The Users Locker Room -------------------------------------------------------------------------------
...
async def search(ctx, *, name):
    ...
    em = discord.Embed(title="Basketball Bot Search", color=discord.Color.red())
    em.add_field(name="All active NBA players with the name `" + name + "`:", value = info, inline=False)
    await ctx.send(embed = em)

# b!stats [player] to get players stats --------------------------------------------------------------------------------
@client.command()
...
async def stats(ctx, *, name):
    ...
    # Player Stats
    em = discord.Embed(title="Basketball Bot Stats", color=discord.Color.red())
...
# b!info [player] to find the info of a player -------------------------------------------------------------------------
@client.command()
@commands.cooldown(1,5,commands.BucketType.user)
async def info(ctx, *, name):
    ...
    # Player Info
    em = discord.Embed(title="Basketball Bot Info", color=discord.Color.red())
...
async def payday(ctx):
        ...
        em.add_field(name="Description:", value="Gives the user a random amount of money", inline=False)
...
# Super Secret Token ---------------------------------------------------------------------------------------------------
client.run('ODU4NTM5MDU1NDA2MTIwOTYw.YNfmzQ.AVH4gf47PZx4Ds23XKQSQXWLl-Y')
